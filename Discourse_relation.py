...
def discourse_relation(sentence):
    ...
    #case 3 for "कुल मिलकर "    
    elif check3: 
        for i in sentence_in_list:
            if i == "कुल":
                sentence_in_list[sentence_in_list.index(i)] += (" " + "मिलाकर")
            if i == "मिलाकर":
                sentence_in_list.remove(i)
        
        for i in sentence_in_list:
            if i == "कुल मिलाकर":
               return sentence_in_list[sentence_in_list.index(i)+1: ]
               
        # return sentence if no conjunction occur in sentence     
        return sentence
